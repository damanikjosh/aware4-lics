...
#         self.publisher1_ = self.create_publisher(VehicleCommand, '/vehicle1/VehicleCommand_PubSubTopic', 10)
...
#         #self.subscriber1_
#         self.vehicle_status_sup = self.create_subscription(VehicleStatus, '/vehicle1/VehicleStatus_PubSubTopic', self.vehicle_status_listener_callback, 10)
#         self.vehicle_gps_sup = self.create_subscription(VehicleGpsPosition, '/vehicle1/VehicleGpsPosition_PubSubTopic', self.vehicle_gps_listener_callback, 10)
...
#         lat, lon = xy2latlon(240, 90)

#         self.tgt_pos = [lat, lon, 5.000]
...
class Vehicle(Node):
    def __init__(self, id, waypoints):
        ...
        self.id = id
        self.publisher = self.create_publisher(VehicleCommand, '/vehicle%d/VehicleCommand_PubSubTopic' % id, 10)
        self.vehicle_local_pose_sup = self.create_subscription(VehicleLocalPosition, '/vehicle%d/VehicleLocalPosition_PubSubTopic' % id, self.listener_callback, 10)
        self.vehicle_status_sup = self.create_subscription(VehicleStatus, '/vehicle%d/VehicleStatus_PubSubTopic' % id, self.vehicle_status_listener_callback, 10)
        self.vehicle_gps_sup = self.create_subscription(VehicleGpsPosition, '/vehicle%d/VehicleGpsPosition_PubSubTopic' % id, self.vehicle_gps_listener_callback, 10)
    ...
    def vehicle_status_listener_callback(self, msg):
        ...
        if self.exit_value == -1 :
            if self.nav_state != 0 and self.arming_state == 1 and self.mode == 0 and self.armed_time <= 0:
                ...
                print("first takeoff")
                self.mode = 2
            elif self.nav_state == 17 and self.arming_state == 2 and self.mode == 2 and self.gps_pos[2] >= self.takeoff_value:
...
def main(args=None):
    ...
    vehicle1 = Vehicle(1, [[lat1, lon1, 5.0],
                           [lat2, lon2, 5.0],
                           [lat3, lon3, 5.0]])
